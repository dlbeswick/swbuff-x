...
(define-minor-mode swbuff-mode
  "Minor mode enabling swbuff key commands."
  :keymap
  '(("\C-q" . swbuff-kill-this-buffer))
  :lighter " swbuff"
  )

(define-globalized-minor-mode swbuff-global-mode swbuff-mode (lambda () (swbuff-mode 1)))
...
`swbuff-post-command-hook'"
  ...
  (setq swbuff-last-buffer-list (cl-copy-list swbuff-initial-buffer-list))
  (swbuff-global-mode -1)
  ...
  (let ((buffer (get-buffer swbuff-status-buffer-name))
	...
	(when (and swbuff-initial-buffer swbuff-current-buffer)
	  (set-window-prev-buffers
	   swbuff-initial-window
	   (mapcar
		(lambda (x)
		  (cl-find x (window-prev-buffers swbuff-initial-window) :key 'car))
		swbuff-last-buffer-list))
...
BCURR is the buffer name to highlight."
  (let* ((blist swbuff-initial-buffer-list)
	 ...
	 (adjust (and
			  ...
			  (or (eq swbuff-status-window-layout 'adjust)
...
The status window shows the list of switchable buffers.	 The
active one is highlighted using `swbuff-current-buffer-face'.  It
